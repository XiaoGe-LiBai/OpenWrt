...
if [ "${REPO_BRANCH#*-}" = "23.05" ]; then
   sed -i "s/DISTRIB_DESCRIPTION=.*/DISTRIB_DESCRIPTION=\"OpenWrt R$(TZ=UTC-8 date +'%y.%-m.%-d') (By @Jejz build $(TZ=UTC-8 date '+%Y-%m-%d %H'):$(TZ=UTC-8 date +%M | sed -e 's/4/5/g'))\"/g"  package/base-files/files/etc/openwrt_release
   ...
   sed -i "s/OPENWRT_RELEASE=.*/OPENWRT_RELEASE=\"OpenWrt R$(TZ=UTC-8 date +'%y.%-m.%-d') (By @Jejz build $(TZ=UTC-8 date '+%Y-%m-%d %H'):$(TZ=UTC-8 date +%M | sed -e 's/4/5/g'))\"/g"  package/base-files/files/usr/lib/os-release
